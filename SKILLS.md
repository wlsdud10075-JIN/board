...
- 🔒 **필수 락 = 차값 · 판매가**(셀프검차 전용, 2026-08-10 Jin). 둘 중 하나라도 비면 **구매확정이 막힌다**.
  차값이 없으면 car-erp 가 422(`required_without`), 판매가가 없으면 car-erp 가 판매 pre-fill 을 통째로 보류해
  (`sale_price>0 && rate>0`) **ERP 판매탭이 빈 채로 생긴다** — 관리가 나중에 손으로 채우면 board 값과 갈린다.
