...
- **§11 요청·확인 신호**(카톡 대체) = `POST/GET /api/internal/board/requests`. 매입 행마다 **[계약금]/[매입잔금]**(차량 1대 + 금액), 판매 바이어 블록에서 차량 체크 후 **[판매대금확인]**(바이어 1 + N대). 권위 = car-erp `board-portal-api.md §11`.
  ...
  - **대표계약금 = 세 번째 매입 type**(`purchase_deposit_ceo`, 2026-09-17 배포 master `fd631e2` ↔ car-erp `ae14f24`). 보내는 내용은 계약금과 같고(차량 1대 + 금액), **다른 점은 수신자가 대표 고정 · 시각 규칙 무시** 하나뿐이다. 담당자가 휴가·외근이면 평시 라우팅상 담당자에게만 가 있어 **사람이 계속 확인해 줘야 했던** 것을 없앤 신호다.
    - 여기도 **별개 type** 이어야 하는 이유는 위와 같다(멱등키 `(vehicle_id, type)`). 같은 차에 `purchase_deposit` open + `purchase_deposit_ceo` open 이 **동시에 성립**하고, **board 는 막지 않는다** — 평시 요청이 안 먹혀 대표에게 다시 보내는 게 용도다. 중복 송금 방지는 **뱃지 병렬 표시만**(Jin 2026-09-17 결정 — ERP 화면에 추가 경고를 넣자는 안은 채택 안 됨).
    - 🚫 **board 가 시각을 판정하지 않는다** — 근무시간·긴급도·수신자 힌트를 payload 에 싣지 않는다. 가드 = `test_ceo_deposit_request_is_a_separate_type_with_amount_and_no_time_hint`.
    - ℹ️ ERP 라우팅은 **시각 규칙 표가 아니라 코드 분기**다(car-erp 실측 회신). 기존 시각 규칙 3행은 `types` 가 비어 **전 신호에 적용**되는 하위호환이라, 표로 풀면 평일 낮엔 담당자 행이 걸려 대표에게 **안 가고**, 넓히면 **기존 3종(판매대금확인 포함)까지 평일에 대표로** 간다. board 가 "시각 규칙 한 줄이면 되는 일"로 오해하지 말 것.
...
### 14-15. 첨부사진 계좌 추출 — 제안까지만, 판정은 코드가 (2026-09-10 배포)

영업이 딜러에게 받은 사진에 계좌가 있는데 **손으로 옮겨 적지 않아** `payee_account` 기재율이
**19%(6/31)** 였다. 그 결과 ①연동 B 가 계좌 없이 나가고 ②**입금요청 알림톡이 「계좌 미등록」으로 나갔다**
(car-erp `BoardRequestController::payeeLine()` 이 계좌를 본문에 싣는다 — 계약금·매입잔금만, 판매대금확인엔 안 싣는다).
사내 GPU PC 비전 모델(`qwen2.5vl:7b`)로 읽어 **후보로 제안**하고, 사람이 `/auction` 드로어에서 확정한다.

- **흐름** = 사진 첨부 → **[📷 계좌정보 찾기]**(`savePayee`) → 15~20초 → 후보 블록(근거 사진 썸네일 +
  [차값에 적용]/[매도비에 적용]) → 눈으로 확인 → **[구매확정]**. ⚠️ `conclude()` 가 `applyPayee()` 를
  호출하므로 **적용 후 곧바로 확정하면 계좌가 함께 저장된다**(별도 저장 버튼 불필요).
- 🚫 **자동 기입 없음.** 적용은 입력칸까지만. 신뢰도가 높아도 사람이 본다.
- 🚨 **계좌는 구매확정 *전에* 넣어야 한다** — car-erp 는 멱등 경로에서 `payee_*` 를 갱신하지 않는다
  (push-once, §12). 확정 뒤에 넣으면 그 차의 계좌는 ERP 에 영영 없다. 그래서 추출 중에는 확정을 막는다
  (+ **[계좌 없이 확정하기]** 탈출구 — 계좌는 원래 필수가 아니다).

**판정은 코드가 한다(`App\Services\PayeeExtractor`) — 프롬프트가 아니라.**

| 규칙 | 처리 |
|---|---|
| 고객센터(1566/1588/1544…) · 자릿수 10~17 밖 · 숫자 외 문자 | 버린다 |
| **0 으로 시작(10~11자리)** | 🚨 **버리지 않는다** — `phone_like` 경고만 |
| 매도비 라벨(매도비·이전비·수수료·알선)이 없는 fee_account | **car 로 돌린다** |
| 은행명(`WOORIBANK`·`IBK 기업은행`) | `koreanBanks` 13종으로 정규화 |
| 예금주 ↔ 등록증 소유자 불일치 | 버리지 않고 `owner_mismatch` 경고 |
| 같은 계좌가 여러 장에 | 후보 1개로 합치고 근거 사진만 누적 |

🚨 **평생계좌를 전화번호로 오인해 버리지 말 것.** 은행이 전화·휴대폰 번호로 만들어주는 계좌가
**실제 송금 계좌**다(IBK·농협). 처음에 「0 으로 시작 = 전화번호」로 버렸다가 딜러가 보낸
「차대금 계좌 기업은행 031 296 5454」를 통째로 날려 *"계좌만 있는 사진인데 못 찾는다"* 가 됐다.

🚨 **프롬프트에 이중 부정을 쓰지 말 것.** `fee_account` 를 「그런 라벨이 실제로 적혀 있을 때만 채우고,
없으면 null」로 썼더니 모델이 보수적으로 굴어 **라벨이 또렷한 두 계좌 사진에서 매도비를 통째로 빠뜨렸다**.
「여러 개 보이면 **하나도 빠뜨리지 말고**」로 바꾸자 둘 다 잡았다. 프롬프트는 다 담게 하고 판정은 코드에.

⚠️ **이 계열 회귀는 테스트로 안 잡힌다**(테스트는 가짜 응답을 쓴다). `PROMPT` 상수를 고치면
**운영 표본으로 재검증**할 것. 검증 방식 = board 서버에서 파이썬으로 `/api/generate` 직접 호출
(`php artisan tinker <파일>` 은 **파일을 실행하지 않고 대화형으로 멈춘다** — `--execute` 를 쓸 것).

**인프라 (RTX 2070 8GB 실측)**
- **1100px + `num_ctx` 4096 이 상한.** 1500px 는 비전 토큰 3300~4100 → `exceed_context_size_error` 400.
  토큰 ≈ `가로 × 세로 / 770`. `num_ctx` 는 **반드시 명시** — 이 모델 기본값이 **128000** 이라 안 주면 VRAM 초과.
- 🚨 **VRAM 여유가 성능을 지배한다**: 같은 이미지가 여유 26MiB 에서 **45.4초**, 448MiB 에서 **2.0초**.
  `ollama ps` 의 `100% GPU` 는 **가중치 위치만** 말한다(연산 버퍼는 안 보인다) — 느려도 폴백으로 안 잡힌다.
- 8GB 에 **비전 모델과 챗봇은 공존 불가**(각 5.5GB). 현재 gpu-office = `qwen2.5vl:7b` 단독 상주,
  3사 ERP 챗봇 off + 03:00 색인 정지(`llm-poc\SYNC-PAUSED`). 되살릴 때 = **색인 재개 → 1회 실행 → 챗봇 on**,
  둘 다 쓰려면 **GPU 증설(16GB 최소선)**.
- 🚨 **`.env`·config 변경 뒤 큐 워커 재시작 필수**(`supervisorctl restart board-worker` /
  `board-ssancar-worker`). 워커는 부팅 시 config 를 들고 있어, 배포 직후 켠 설정을 못 읽고 Job 이
  **로그 한 줄 없이 무동작 종료**한다(실제 발생 — 화면은 영영 "찾는 중"). `jobs`·`failed_jobs` 0 + 로그 0 = 이 패턴.
- 그래도 갇히지 않게 **`payee_extraction_at` + 5분 타임아웃**(`payeeExtractionInFlight()`)으로 확정을 푼다.
- ⚠️ `phpunit.xml` 에 `BOARD_PAYEE_EXTRACT_ENABLED=false` 고정 — 없으면 로컬 `.env` 를 물려받아
  "사진 올리며 바로 구매확정" 테스트가 가드에 걸려 깨진다.

**UI 함정 (실사용에서 나온 것)**
- 못 찾았으면 **그렇다고 말한다**. 아무것도 안 그리면 사용자는 화면이 멈춘 줄 알고 기다린다(3분 실측).
- 탈출구 버튼을 **회색 밑줄 글씨로 두지 말 것** — 막힌 사람이 빠져나갈 유일한 문이다.
- 🚨 **[매입취소]가 「닫기」로 읽혀 매물이 실제로 취소됐다.** 라벨 `failed_encar` = 「취소」→「매입취소」,
  하단에 **[닫기]** 별도 배치(✕ 는 우상단에만 있어 버튼 줄에서 눈에 안 들어온다).

**운영 안내(담당자 교육)**: 계좌가 보이는 사진(통장사본·계좌 문자 캡처)을 넣는다 / 명함처럼 전화번호만
...
- **탭 = 상태 묶음**(`PurchaseListing::TAB_STATUSES`) — 전체·진행중·현지확인대기·검차완료·회신대기·수락·
  낙찰/확정·ERP전환완료·종료(거절+유찰). **기본 = 「전체」 + 30건**(2026-09-11 Jin) — 화면 성격은
  전과 같게 두고(전체가 보인다) **느려지던 원인인 전량 로드만** 페이지네이션으로 끊는다. 좁혀 보려면 탭을 누른다.
  ⚠️ 기본을 「진행중」으로 되돌리지 말 것 — 한 번 그렇게 잡았다가 Jin 이 전체로 바꿨다(목록의 성격 문제).
...
- **페이지당 건수** = 10/20/30/50/100 + **「건수만」(0)**. car-erp 차량관리와 같은 규칙·같은 수법 —
  ...
  ⚠️ `#[Url]` 이라 `?perPage=7` 처럼 아무 값이나 들어온다 → **화이트리스트 밖이면 기본(30)으로 되돌린다**
  (`updatedPerPage()` + `listings()` 양쪽 — URL 로 바로 들어오면 `updated*` 훅이 안 도는 경로가 있다).
- ⚠️ 탭 전환·건수 변경·**신규 등록**은 **반드시 `resetPage()`** — 3페이지를 보다 탭을 바꾸면 빈 페이지가 뜨고,
  2페이지를 보다 등록하면 새 행(=`latest()` 라 1페이지 맨 위)이 안 보인다.
- ⚠️ **`?tab=` 도 `#[Url]`** 이라 `setTab` 을 안 거치고 들어온다 — 모르는 값이면 **조건 없이 전량**이 뜨는데
  칩은 하나도 안 켜져 이유를 알 수 없다. `perPage` 와 **같은 자리(`listings()` 맨 위)**에서 되돌린다.
- ⚠️ 필터 셀렉트는 **`input-filter`**(2026-09-11 car-erp 에서 이식). 🚫 `input-base` 를 쓰지 말 것 —
  **`width:100%`** 라 필터줄에서 폭을 다 먹고 **탭 칩을 밀어낸다**(Jin 실측: "드롭박스가 너무 커서 잘린 것 같다").
  ⚠️ **Tailwind `w-auto` 로는 못 이긴다** — 커스텀 클래스가 유틸리티 레이어 **뒤에** 와서 `width:100%` 가 이긴다.
  car-erp 마크업을 옮겨올 땐 **그 클래스가 board 에 있는지부터** 보고, 없으면 정의째 이식한다.
- ⚠️ **모바일이 주 사용처다**(Jin 2026-09-11). 탭줄 = **가로 스크롤 칩**(`overflow-x-auto whitespace-nowrap`) —
  줄바꿈시키면 9개가 2~3줄이 되어 목록이 화면 밖으로 밀린다.
...
### 14-18. 매도비는 **적은 것만 간다** (2026-09-11 Jin — 자동 기입 폐지)

매도비가 **세 곳에서 자동으로 붙고** 있었다. 셋 다 사람이 적은 적 없는 숫자다.

1. `PurchaseListing::sellingFeeKrw()` — 입력이 없으면 **차값 있는 모든 차**에 고정 440,000 을 실어 ERP 로 보냈다.
2. `/auction` 드로어 — 열 때 매도비 칸에 440,000 을 미리 채웠다(2026-08-10 "입력을 줄인다" 결정 → **오늘 뒤집힘**).
3. 🚨 `/listings` 추가 폼 미리보기(`calcCarPrice`) — 차량금액에 **+440,000** 을 더해 보여줬는데, 저장되는
   `final_price`(모델 `totalKrw()`)에는 매도비가 없다 ⇒ **화면 숫자와 원장 숫자가 매도비만큼 달랐다.**
   Model A 상 판매가는 매도비 제외(회사 부담)가 맞으므로 **화면을 원장에 맞췄다**(원장 불변).

- **지금 규칙**: 적으면 그 값, 안 적으면 `null`(payload 에서 빠짐 → ERP `vehicles.selling_fee` 는 기본값 0).
  `config('board.sales_fee')` 는 **제거**했다(읽는 곳 0). 운영 `.env` 의 `BOARD_SALES_FEE` 가 남아도 무해.
- **입력 자리** = `/auction` 드로어 금액 칸, **출처 무관 공통**. 예전엔 **셀프검차매입에만** 있어서 다른 차는
  영업이 손댈 방법이 없었다(Jin: "매도비 쓰는 게 어디쪽에 있지 로컬에서 못 찾겠네").
  ⚠️ 저장 코드도 **셀프검차 분기 안에만** 있었다 — 칸만 옮기고 저장을 안 옮기면 값이 조용히 0 으로 남는다.
- ⚠️ **`lte:car_cost` 는 셀프검차 전용** — 거기선 매도비가 **차값에 포함**돼 있어 넘을 수 없다(넘으면 매입가가
  0 으로 깎인다). 다른 출처는 **회사 부담 별도**라 차값과 무관하므로 그 규칙을 걸지 않는다.
