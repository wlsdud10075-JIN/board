...
/**
 * mysqldump 으로 board DB 를 storage/backups/db/ 에 백업 (car-erp 미러).
 * 배포 직전(deploy.yml) + 야간 cron(03:00, routes/console.php)에서 사용.
 * DB_BACKUP_DISK(s3) 설정 시 off-instance 업로드.
 */
class BackupDatabase extends Command
{
    ...
    protected $description = 'mysqldump 으로 board DB 를 storage/backups/db/ 에 백업.';
    ...
    public function handle(): int
    {
        ...
        if (! $database) {
            $this->error('config/database.php 에서 DB 이름을 읽지 못했습니다.');
    ...
    /** DB_BACKUP_DISK(s3) 설정 시 원격에도 업로드. 실패해도 로컬 백업은 유효 → SUCCESS 유지. */
    private function maybeUploadToRemote(string $localPath, string $filename): void
    {
        ...
        try {
            ...
            Log::critical('DB 백업 원격 업로드 실패 — 원격 백업본 없음', [
                'disk' => $disk, 'file' => $filename, 'error' => $e->getMessage(),
            ]);
        }
    }
