                ...
                <div class="card-sm mb-2" wire:key="recv-{{ $loop->index }}" x-data="{ open: false }">
                    ...
                    <button type="button" class="w-full text-left" @click="open = !open">
                        ...
                        <div class="mt-1.5 flex items-center gap-2 pl-5">
                            <div class="h-2.5 flex-1 overflow-hidden rounded-full bg-gray-200">
                                <div class="h-full rounded-full bg-amber-500" style="width: {{ $share }}%"></div>
                            </div>
                            <span class="shrink-0 text-[11px] font-semibold text-gray-500">{{ number_format($share, 1) }}%</span>
                        </div>
                    </button>
                                                    ...
                                                    @elseif ($k === 'unpaid_krw' && is_numeric($val))
                                                        ...
                                                        @if ($ratio !== null && is_numeric($ratio))
                                                            ...
                                                            <div class="mt-1 flex items-center gap-1.5">
                                                                <div class="h-1.5 w-24 overflow-hidden rounded-full bg-gray-200"><div class="h-full rounded-full" style="width: {{ $pct }}%; background: hsl({{ round($hue) }}, 65%, 50%)"></div></div>
                                ...
                                <div class="rounded-md border border-gray-100 bg-gray-50 px-2.5 py-2">
                                    ...
                                    @if ($ratio !== null && is_numeric($ratio))
                                        ...
                                        <div class="mt-1.5 flex items-center gap-1.5">
                                            <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-gray-200"><div class="h-full rounded-full" style="width: {{ $pct }}%; background: hsl({{ round($hue) }}, 65%, 50%)"></div></div>
                                            <span class="text-[10px] text-gray-500">{{ __('portal.recv_unpaid_pct', ['pct' => number_format($pct, 0)]) }}</span>
                                        </div>
                                    @endif
                                </div>
