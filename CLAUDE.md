...
- **c_no 채우는 경로 = 출처별 분기**: ssancar.com 클릭 유입(→ respond.io 채팅에 c_no 자동 도착)은 **연동 A inbound 가 자동 채움**(ssancar 는 우리 소유라 채팅 링크에 c_no 보장 가능). 엔카·기타 출처(ssancar 미경유)는 **c_no 없음 = NULL** 정상, 예외적으로만 영업 수동입력. → c_no 는 "ssancar 유입 거래의 바이어 추적 실"이지 모든 listing 의 필수값 아님.
