...
**(e) car-erp 입금계좌 = car-erp 전용 (결정)**
- 매입 정산 입금계좌(금융 PII)는 **board 미보유** — board 의 "RRN·개인정보 미보유 + APP_KEY 분리" 정당성 유지. 입금은 매입 *확정 후* = car-erp 도메인.
- 연동 B push = 차량+영업+c_no 만. **계좌·입금은 car-erp 재무가 매입탭에서 입력**(기존 워크플로우). board 경매담당 → car-erp 재무 **핸드오프 절차만 문서화**. (passthrough 는 이중입력 고통 실증 시에만 검토.)
