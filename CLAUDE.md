...
- **재고매입(바이어 미정)** — 차값이 쌀 때 바이어 없이 미리 사두는 매입(2026-09-08 Jin 방향 확정, **미착수**): `/auction` 에 토글 → `buyer_id: null` 로 전송 → ERP 「일반재고」로 잡히고 이후는 **ERP 화면에서 진행**(바이어 지정 시 선적전으로 이동). ⏳ **car-erp 회신 대기** — 질문·제안 payload·위험고지 = `meetings/handoff-carerp-stock-purchase-no-buyer.md`.
  - ⚠️ 바이어를 안 고르면 `buyer_id` 가 null 이라 **매입 등록 락 판정이 성립하지 않는다**(= 2026-08-10 에 바이어를 필수화한 바로 그 이유). 재고매입은 락을 건너뛸 수밖에 없으므로 **ERP 가 「나중에 바이어 지정할 때」 락을 거는지**가 이 기능의 전제다 — 안 걸면 구멍이 문 하나 옆에서 다시 열린다.
  - ⚠️ 판매가를 **비워두는 것만으로는 부족하다** — Job 이 `차량금액 ÷ 환율` 로 `sale_price` 를 **자동 파생**한다(`SyncWonListingToCarErp.php`). 재고매입은 파생을 끄고 명시적으로 비워 보내야 한다(ERP 「일반재고」 판정이 `sale_price` 기준일 경우 특히).
