...
- **재고매입(바이어 미정)** — 차값이 쌀 때 바이어 없이 미리 사두는 매입(2026-09-08 Jin): `/auction` 드로어 토글 → 연동 B **v5**(`buyer_undecided`)로 car-erp 에 전달 → 이후는 **ERP 화면에서 진행**(거기서 바이어를 붙이면 「선적전」으로 이동). **board dev 구현 완료·운영 미배포**. car-erp 는 **v5 배포 완료**(2026-09-08). 규칙·함정 = `SKILLS.md §12` v5 절, 인계·회신 = `meetings/handoff-carerp-stock-purchase-no-buyer.md`.
  - 권한·출처 제한 **없음**(Jin 확정) · 판매측 금액칸은 토글 시 숨김 · **차값 게이트와 딜러 첨부 필수는 그대로** · 변경은 감사로그(`buyer_undecided` ∈ AUDITED).
  - 🚨 **판매가를 안 적는 것만으로는 안 된다** — Job 이 `차량금액 ÷ 환율` 로 파생한다. ERP 재고 분류는 `sale_price` 하나로 갈리므로(>0 = 「선적전」) 판매측을 **Job 에서 강제로 비운다**. `buyer_id` 도 같이 비운다 — ERP **재전송 경로엔 매입 등록 락 게이트가 없어서**(회신 Q3) 컬럼에 남은 바이어가 나중에 실려 나가면 락 우회로가 된다.
  - ⚠️ **신규 재고매입 차는 「지급대기」에 먼저 앉는다**(매입 미지급). 매입대금 확정 지급 후 「일반재고」로 이동하고, 그 전까지는 ERP 재고관리 **「전체」 탭에도 안 보인다**(전체 = 매입완납). 버그로 읽기 쉬운 자리라 사용자에게 먼저 알릴 것.
  - 🅿️ 선택 후속 = board `/portal` 재고 행에 「재고매입」 뱃지(ERP 가 `buyer_undecided` 를 이미 실어 보낸다 — board 는 읽어 그리기만).
