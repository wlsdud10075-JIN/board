...
- inspection 드로어의 S3 사진을 바이어 WhatsApp 으로 전송(outbound) + 회신 수신(inbound webhook → `buyer_verdict` 자동 업데이트; accepted 전이는 기존 `buyer_verdict='accepted'` 가드와 그대로 맞물림).
- 신규 구성: **`routes/api.php` + `WebhookController` + HMAC 서명검증**(현재 bootstrap/app.php 는 web 만 라우팅), `RespondIoService`(메시지 템플릿·presigned URL 전송).
- 신규 컬럼: **`c_no`**(ssancar.com 매물번호 = 전 시스템 조인키, nullable·indexed·**non-unique** — 중복키 아님; 지금은 영업 수동입력, 연동 A 때 자동 채움) + **`respond_contact_id`/`respond_conversation_id`**(opaque 외부 ID — buyer_name 문자열 대체). **`phone_hash` 등 전화 파생값은 추가 안 함**(contact_id 로 식별 충분 + board 는 PII 최소보유 원칙).
- **선행 = 도메인 + HTTPS**(서브도메인 + certbot)뿐. **DPA/SCC 서명은 2026-06-10 완료 → 법적 선행조건 done**(다음 단계는 도메인+HTTPS + 아래 기술필터).
...
- 테스트: HTTP fake 로 "accept webhook → buyer_verdict=accepted", "중복 webhook 무시(external_event_id)" 추가.
