...
1. **listings**(영업): **상태 탭**(전체·진행중(기본)·현지확인대기·검차완료·회신대기·수락·낙찰/확정·ERP전환완료·종료) + **페이지당 건수(10/20/30/50/100·건수만) + 페이지네이션** · 매입예정 추가(출처 토글·TimeGate 가드, 차량번호/소유자/차값/할인 가로 grid) + 본인 글 행클릭 편집 드로어. 드로어 하단에 **본인 등록건 삭제**(soft delete — ERP 미연동 + accepted 이전 상태만, 아래 「완료된 로드맵」 참조).
