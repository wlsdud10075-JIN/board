...
- **영업담당자 매칭 = 이메일 기준** (숫자 id 입력 폐기 — DB 봐야 알아서 UX 불가):
  - car-erp 가 board 가 보낸 `salesman_email` 로 `salesmen` 조회 → `vehicles.salesman_id` 자동 지정.
  - `salesman_email` = board 영업의 **`car_erp_salesman_email`(오버라이드) 있으면 그것, 없으면 로그인 이메일.** 즉 로그인=car-erp 이메일이면 자동, 다르면 `/users` 에서 car-erp 이메일만 적으면 됨. `car_erp_salesman_id`(숫자) 컬럼은 잔존하나 UI 미노출(car-erp overrideId 깊은 폴백용).
...
> 📐 **설계 확정 = `meetings/integration-A-design.md`** (착수 시 스펙). 아래 원래 메모는 그 문서로 일부 정정됨:
> - ⚠️ **board 입장 트리거 = 영업 수동 "승격"(모든 소스 통일, 2026-06-15 대표 (b)안).** 링크 도착·inbound 자체로 listing 자동생성 안 함(노이즈 방지). ssancar 도 c_no 는 자동 *캡처*하되 listing 은 영업이 승격. 자동화 = 승격을 한 클릭으로(c_no/encar_id 자동추출+대화 연결).
> - **유입 3종**: ssancar(c_no)·Encar(`encar_id` 신규, URL 추출)는 차 문의 → 승격 시 board. **설문폼 리드(사람·차 없음)는 respond.io 컨택트로 끝(board 아님).**
> - **스파인 = `respond_conversation_id`**(respond.io 부여, 자체 발번 불필요). 한 컨택트 : 여러 listing. webhook 은 **차 이벤트만** board 로(리드·잡담 제외).
...
- ⚠️ **차값 통화 인식 (2026-06-22 Jin)**: 엔카=원화만, **싼카=가져온 원/미/유로 중 매물표시가 토글로 택1**한 통화를 `car_cost` 에 **그대로 보관**(외화 그대로, `expected_price_currency`). 매물표시가 토글=차값 통화·금액 선택, **금액산정 토글(displayCurrency)=표시만**(차값 불변). KRW 환산은 **계산할 때만** — 단일 경로 `App\Support\Money::toKrw()` ↔ 모델 `carCostKrw/carPriceKrw/totalKrw($usd,$eur)`. KRW 행(기존 전부)은 항등 → 무영향. 차값 표시 = `carCostDisplay()`(통화기호). final_price 스냅샷은 KRW(연동B 무변).
...
- **입력 UX = car-erp 형식 미러**: 은행명 datalist 자동완성(13개 한국은행) + 계좌번호 **은행별 동적 하이픈 마스킹**. `resources/js/app.js` 의 `Alpine.store('koreanBanks')`(car-erp 와 동일 패턴) 사용 — 블록마다 `x-data` + `x-ref` (bank↔account 동일 스코프). 새 JS 라 `npm run build` 필요.
...
- **개인정보 스탠스(개정)**: 판매자/경매장 계좌는 B2B 금융정보 — RRN 같은 최고 레드라인은 아니나 민감. 그래서 **계좌번호 암호화 + 감사로그/표시 최소화**의 *범위 한정 예외*로 보유. (RRN·전화 미보유 원칙은 유지.)
  - ⚠️ **서류 보유 = 추가 범위한정 예외 (2026-06-22, Jin 결정)**: 영업이 board 에 **차량 사진+서류(차량등록증 등)** 를 올려 낙찰 시 연동 B(v2 `attachments[]`)로 car-erp 첨부탭에 자동등록(메신저 제거). 기존 "서류 미보유" 원칙 번복이나 근거 = ① 차량등록증은 **주소·RRN 마스킹본** ② car-erp 가 NICE 로 권위데이터 재등록 → board 보유분은 **참고사본** ③ **실행파일만 차단**(`App\Support\UploadGuard`). 격리 = 전용 S3 prefix(`sales/photos`·`sales/documents`) + 서류는 바이어 전송 제외(§28, `share_to_buyer=false` 강제). 구현 = `listings` 업로드 UI + `inspection_photos.kind`(inspection/sales_photo/sales_document) + 연동 B 송신. 인계(수신측 car-erp) = `meetings/handoff-car-erp-vehicle-attachments.md`. Jin≠대표지만 대표 부재 + 위 근거로 진행.
