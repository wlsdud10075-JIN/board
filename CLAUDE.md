...
- **ssancar 자동전달대기 전이 불발**(2026-07-28 실측): ①ssancar 검차글이 차당 2개(영상글+사진전용글)인데 교차매칭이 `matched:1` 로 사진전용글을 물어 `videos:[]` → 전이 안 됨. **요청서 = `meetings/handoff-ssancar-media-merge.md`**(ssancar 세션 전달 대기, 결정질문 §5-2 = 합산이냐 영상글우선이냐). ②별개 경로 — `SsancarMediaService` `Http::timeout(4)` 인데 콜드 응답 4.8초 실측 → 빈 배열 → 조용히 전이 안 함. **폴러(백그라운드)와 바이어페이지 타임아웃 분리** 필요(board 단독 수정 가능, 미착수). [board-ssancar-inspected-duplicate-entries]
