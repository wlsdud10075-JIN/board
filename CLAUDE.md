...
- ~~**board 앱 챗봇 통합**~~ **✅ 2026-08-03 운영 가동**(master `be425b5`, 두 박스). 로컬 LLM 업무 Q&A **A단계(업무가이드 RAG)만** 이식 — B(미수·채권·자금)는 그 데이터가 car-erp 원장에 있어 board 엔 해당 없음.
  ...
  - Notion 카드 = `scripts/notion-cards/publish.php`. **최초 발행 전용이었음** → `--apply --replace`(재발행) 추가(부모 페이지 유지, 그룹 페이지만 교체). 발행은 **Codex 위임** — 절차 정본 = `C:\xampp\htdocs\CODEX_NOTION_HANDOFF.md` **§9-A2**(⚠️ car-erp §9-A 와 도구가 다르다: board 는 `--verify`·`--card` 없고 `--replace` 전체 재발행이 정상 경로, `audience` 없음). 카드 고칠 일 = `cards.json` 수정 → Codex 에 발행 요청 → **익일 03:00** 색인 반영.
