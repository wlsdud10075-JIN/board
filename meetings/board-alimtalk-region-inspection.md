...
## ⛔ 슬라이스 2 차단 = 지역 배정 모델 미결정 (Jin 2026-07-12)
슬라이스 2(트리거)의 **수신자 해석**이 이 모델에 통째로 달려 있어 **결정 전 착수 불가**:
- **(a) 지역 고정 인원** — 지역마다 상주 검차원(region→user 매핑, 날짜無). 수신자 = 그 지역 고정 인원. 관리 일일배정 불필요. 스케줄 digest 만으로 충분(전날 저녁 자동). 단순·무결근대응약함. → 신규 `region_inspectors`(region·user_id) 또는 users.region. **현 InspectionAssignment(날짜기반) 대체.**
- **(b) 그때그때 배정** — 현 `InspectionAssignment`(date×region×user_id, ≤3). 관리가 매일 배정. 수신자 = 그 날짜 배정자. ⚠️ **전날 저녁 발송하려면 관리가 그 전에 '내일' 배정을 끝내야** 함(안 하면 수신자 0). 유연(결근·부하분산)·일일 마감 부담. **현 코드가 이미 (b) 가정**(§6c).
- 결정 후: 수신자 resolver(내일 날짜 기준) + 스케줄 커맨드 + (b면) 관리 배정화면 저장/반영 버튼.
