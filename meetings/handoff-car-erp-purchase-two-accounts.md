...
## 3. board 측 대응 — ✅ **구현 완료 (dev, 2026-07-03 · 미배포)**
- ✅ 마이그: `purchase_listings` 에 `selling_fee_payee_name/bank/account` 추가(additive, `selling_fee_payee_account` = `encrypted` 캐스트).
- ✅ 입력 UI: 매입예정(`/listings` 추가·편집) + 경매/구매 드로어(`/auction`) 에 **매도비 계좌 입력란**(판매자 계좌 UX 미러 — 은행 datalist + 계좌 하이픈 마스킹, 별도 x-ref 스코프).
- ✅ 송신: `SyncWonListingToCarErp` payload 3필드 추가 + `contract_version: 4`. 로그 `selling_fee_payee_account` `***` 마스킹.
- ✅ SKILLS §12 payload 표 갱신(v4). 테스트 추가(전송 실값 + 로그 마스킹 + 암호화), 181 통과.
- ⏳ **master 미배포** — **car-erp 먼저 배포된 뒤** board v4 master 배포(그전엔 신규 필드 무시=무해, 단 ERP 에 매도비 계좌 안 꽂힘).

---

## 4. 설계 확정 / 미확정
- ✅ **매도비 계좌 = 판매자와 다른 대상 (2026-07-03 Jin).** 매입가 계좌(판매자)와 별개 주체. **영업이 board 에서 직접 기입**(기존 판매자 계좌 입력 UX 미러, 자동 도출/기본값 없음). → board `selling_fee_payee_*` = 순수 사용자 입력 필드.
- ⏳ (car-erp 판단) 매도비 계좌가 항상 있나 vs 선택인가 — board 는 **선택(nullable)**로 구현 예정(공란 허용). car-erp 도 nullable 수신.
- ⏳ (car-erp 판단) 정산씬/매입탭에서 매도비 계좌를 판매자 계좌와 나란히 표시할지 = car-erp UI 범위.
