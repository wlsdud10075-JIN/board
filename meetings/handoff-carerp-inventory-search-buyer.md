...
⇒ **바이어만 없다.** 바이어는 컬럼이 아니라 릴레이션이라 `orWhereHas` 가 하나 더 필요하다.

> ✅ **2026-08-12 배포 완료** (car-erp master `40b5c4b`, 3사 라이브).
> ⚠️ **이 문서가 처음에 `export_buyer_id`(통관 바이어)를 지목한 건 틀렸다** — 재고 행이 **화면에 찍는**
> 바이어는 **`buyer_id`(판매 바이어)** 다. 통관 바이어로 훑으면 *A 로 표시된 행이 B 를 쳤을 때 나온다.*
> ERP 가 바로잡아 **행에 찍히는 그 칸만** 훑는다 ⇒ **보이는 것과 찾히는 것이 같다**(응답 `buyer` 필드와 동일 값).
