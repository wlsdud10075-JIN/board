...
| `transport_fee` | **판매통화(sale_currency)** | `transport_fee` | 운임비. ⚠️ **car-erp 계산이 판매통화 가정**(아래 버그) → board가 판매통화로 **환산해서** 보냄 |
...
## ⚠️ 발견된 버그 — 운임비 통화 (2026-06-23, v3 수신기 이미 구현됨)
car-erp v3 수신기가 이미 있음(SUPPORTED_VERSIONS=[1,2,3]). 단 `PurchaseSyncController:138` 가 `$vehicle->transport_fee = $data['transport_fee_usd']` 로 **USD 값을 raw 저장**하는데, **판매합계 계산은 판매통화 가정**(`Vehicle.php:1201` `sale_price + transport_fee + …` → × `exchange_rate` 단일 환율). → **EUR 판매 차량에서 운임비가 USD인데 EUR 환율로 곱해져 ~16% 부풀고 미수금 틀어짐.**
- 근거: 엑셀 판매총합계 = 판매금액 + **운임비(판매통화 환산)** + 커미션 (운임 USD 아님). 기존 ImportVehicles 도 판매통화 컬럼 매핑. 컬럼 주석 "(USD)"는 옛 표기.
- **수정**: payload 필드 `transport_fee_usd` → **`transport_fee`(판매통화)** 로 받고 그대로 저장. board 가 `shipping_usd × (USD환율/판매환율)` 로 환산해 보냄(USD판매=×1, EUR판매=USD/EUR 크로스, KRW판매=×USD환율).

## 체크리스트 (car-erp)
- [ ] **운임비 통화 버그 수정** (line 138: `transport_fee_usd` raw → `transport_fee` 판매통화 수신)
- [ ] 대표 승인 (무수정 예외 확장)
