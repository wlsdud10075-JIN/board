...
> ## ✅ 2026-09-17 양쪽 배포 완료 — 이 문서는 「긴 스펙·재개 절차」용으로만 남는다
> - car-erp 3사 배포 `ae14f24`(type 수용·대표 직행·뱃지 amber 실측) → board master `fd631e2`(두 박스, db:backup ✓, Nothing to migrate).
> - **§4-1·§4-2 질문은 해소됐다**: BizM 재검수 **불필요**(본문 변수가 `#{건수}`·`#{요청내역}` 둘뿐이라 신호 종류가 본문을 안 바꾼다) / 입금요청 알림톡은 **이미 실발송 중**(heymanerp sent 95, 마지막 2026-09-16).
> - **§8 회신 = 끝남.** Jin 결정 = 수신자는 시각 규칙 fallback(대표) 재사용 · 중복 방지는 뱃지 병렬 표시만.
> - ERP 라우팅은 시각 규칙 표가 아니라 **코드 분기**(표에 얹으면 기존 3종까지 평일에 대표로 간다).
> - 남음 = **실거래 e2e 1건**. 규칙·함정 요약 = board `SKILLS.md §14-2`.
